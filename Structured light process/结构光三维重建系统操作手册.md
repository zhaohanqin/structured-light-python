...
- **相位解包裹核心算法** (unwrap_phase.py)：实现了多种相位解包裹算法
- **图形用户界面** (unwrap_phase_ui.py)：提供了友好的用户界面进行相位解包裹
- **示例代码** (unwrap_phase_example.py)：展示了如何使用相位解包裹API

### 操作流程（相位解包裹）

#### 使用命令行工具

1. **准备条纹图像**：确保已采集多频率相移条纹图像
2. **运行解包裹程序**：

   ```bash
   cd unwrap_phase
   python unwrap_phase.py --input ./test_images --output ./results --method heterodyne
   ```

3. **查看结果**：解包裹后的相位图会保存在输出目录中
...
2. **加载图像**：通过界面加载条纹图像
3. **选择参数**：设置解包裹方法、频率等参数
4. **执行解包裹**：点击"执行解包裹"按钮
5. **保存结果**：将解包裹后的相位图保存为`.npy`文件或图像文件

### API参考(相位解包裹)

#### 相位计算 (unwrap_phase.py)

```python
# 计算包裹相位
calculate_wrapped_phase(images, steps=4)
```
...
```python
# 多频率异质法解包裹
heterodyne_unwrapping(wrapped_phases, frequencies)
```

- **参数**:
  - `wrapped_phases`: 不同频率的包裹相位列表
  - `frequencies`: 对应的频率列表

- **返回值**:
  - `unwrapped_phase`: 解包裹后的绝对相位图

```python
# 格雷码辅助解包裹
graycode_unwrapping(wrapped_phase, graycode_images, threshold=128)
```
...
```python
# 保存解包裹相位
save_unwrapped_phase(unwrapped_phase, file_path, format='npy')
```

- **参数**:
  - `unwrapped_phase`: 解包裹相位图
  - `file_path`: 保存路径
  - `format`: 保存格式，'npy'或'png'等图像格式
...
```python
# 加载解包裹相位
load_unwrapped_phase(file_path)
```

- **参数**:
  - `file_path`: 文件路径，支持'.npy'和图像格式
